...
exports.simpleFooter = function (hex, title, description) {
    return new discord.MessageEmbed()
        ...
        .setTitle(title)
        .setDescription(description)
        .setFooter(`Voyager ${package.version} - Created by the r/Lab_path Team`, config.links.voyagerIcon)
}

// Reddit post without image
exports.redditPostNoImage = function (hex, title, author, url) {
    return new discord.MessageEmbed()
        .setColor(`#${hex}`)
        .setTitle(title)
